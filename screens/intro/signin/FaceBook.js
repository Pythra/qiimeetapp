  ...
  try {
    ...
    if (result.type === 'success' && result.params.code) {
      ...
      if (tokenResponse.access_token) {
        // For signin, we should check if user exists and navigate accordingly
        ...
        try {
          const userCheckResponse = await fetch(`${process.env.API_BASE_URL || 'https://your-api-url.com'}/auth/check-user`, {
            method: 'POST',
            headers: {
          ...
          if (userCheckResponse.ok) {
            ...
            if (userData.exists) {
              // User exists, navigate to MainTabs
              navigation.navigate('MainTabs');
            } else {
              // User doesn't exist, navigate to Onboarding
